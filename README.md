...
- **단일 구현을 유지한다.** 같은 일을 하는 코드 경로를 둘로 두지 않는다. 구조를 바꿀 때는 이전 구현을 남기지 않고 대체한다.
- **조용한 실패를 만들지 않는다.** 오류를 삼키고 기본값으로 넘어가는 대신, `status`와 `diagnostics`로 드러낸다.
- **알려진 테스트를 통과시키기 위한 특수 분기를 두지 않는다.** 고정된 테스트셋은 시간이 갈수록 낡는다고 보고, 새로운 입력과 경계 사례에서도 같은 규칙이 적용되는지를 기준으로 삼는다.
- **통과율만 보지 않는다.** 실패 원인, 검증 범위의 한계, 아직 확인하지 못한 일반화 주장을 결과와 함께 구분해 기록한다.
