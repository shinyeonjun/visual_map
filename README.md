...
개발 모드의 분석 결과는 `backend/code_analysis_engine/tests/dev` 아래 워크스페이스에 저장됩니다. 릴리스 모드에서는 운영체제의 사용자 데이터 폴더 아래 `VisualMap`을 사용합니다.

## 설정

기본 설정은 `backend/code_analysis_engine/config/analysis.default.toml`에 있습니다.
