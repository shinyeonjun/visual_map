...
- Added `release-smoke.ps1` with PE, size, checksum, engine/notices/license and opt-in clean install/start/uninstall checks for local builds.
...
The product uses the MIT license, the database engine is publicly pinned, and public source publication is complete. Official Windows installer distribution and code signing are intentionally out of scope; the locally validated installer is not published as a distributable.
