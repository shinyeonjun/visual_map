...
import type { AnalysisProgress, CodexModel, Project } from '../domain'
...
export function AnalysisProgressPanel({ progress, floating = false }: { progress: AnalysisProgress; floating?: boolean }) {
  ...
  const progressText = hasChunks
    ? `Codex 청크 ${progress.current}/${progress.total}`
  ...
  return <section className={`analysis-progress-panel ${floating ? 'floating' : ''} ${isError ? 'error' : ''}`} aria-live="polite">
    ...
    <div className="analysis-progress-meta"><span>{progress.phase === 'semantic' && hasChunks ? `${progress.current}/${progress.total} 청크 처리됨` : '완료된 단계 기준으로 표시'}</span><span>{elapsedText}</span></div>
  </section>
}
...
export function SetupScreen({
  ...
  const canAnalyze = Boolean(project && codexVersion && model && !isAnalyzing)
    ...
    <div className="setup-shell">
      ...
      <main className="setup-main">
        <div className="setup-intro">
          ...
          <h1>코드베이스를 연결하세요</h1>
          <p>프로젝트와 Codex CLI를 연결하면<br />코드 구조에서 비즈니스 도메인을 찾아 지도로 보여줍니다.</p>
        </div>
        ...
        <section className="setup-card" aria-label="분석 설정">
          ...
          <SetupStep number="02" title="Codex CLI 연결" complete={Boolean(codexVersion)}>
            {codexVersion ? <div className="setup-connected"><strong>Codex CLI 준비됨</strong><span>{codexVersion}</span></div> : <p className="setup-placeholder">설치된 Codex CLI를 확인합니다.</p>}
            <button className="setup-secondary" onClick={onCheckCodex} disabled={isCodexChecking}><Icon name="spark" size={14} />{isCodexChecking ? '확인 중…' : codexVersion ? '다시 확인' : '연결 확인'}</button>
          </SetupStep>

          <SetupStep number="03" title="분석 모델" complete={Boolean(model)}>
            <div className="setup-model-copy"><strong>의미 분석에 사용할 모델</strong><span>이 설정은 분석을 시작할 때 적용됩니다.</span></div>
            <select className="setup-model" value={model} onChange={(event) => void onModelChange(event.target.value)} aria-label="Codex 모델 선택" disabled={models.length === 0}>
              {models.length === 0 && <option value="">먼저 CLI를 연결하세요</option>}
          ...
          {codexError && <p className="setup-error">{codexError}</p>}
          {notice && !codexError && <p className="setup-notice">{notice}</p>}
          <button className="setup-primary" onClick={onAnalyze} disabled={!canAnalyze}>
            <Icon name="spark" size={15} />
            {isAnalyzing ? '프로젝트 분석 중…' : '분석 시작'}
            <span>↗</span>
          </button>
          <p className="setup-footnote">정적 분석 후 Codex가 도메인 이름과 설명을 생성합니다.</p>
        </section>
      </main>
    </div>
  )
