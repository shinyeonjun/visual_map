...
function area(index: number): MapArea {
  return {
    ...
    depth: 0,
    areas: [],
    nodes: [{ id: `node-${index}`, name: `symbol${index}`, kind: "Code", role: "code", definition: null }],
    hiddenNodeCount: 0,
    position: null,
...
function draw(view: MapView) {
  return render(
    <OverviewCanvas view={view} selectedId={null} onSelect={() => undefined} onOpenTrace={() => undefined} />,
  );
}
...
describe("large map canvas", () => {
  it("expands its world for every unplaced area instead of clipping after a fixed number of rows", () => {
    const { container } = draw({
      areas: Array.from({ length: 40 }, (_, index) => area(index)),
      relations: [],
  ...
  it("includes a distant stored position in the world bounds", () => {
    ...
    const { container } = draw({ areas: [distant], relations: [], unattributedAnalysisGapCount: 0 });
