...
export function inspectorAnswer({
  ...
  if (edge) {
    ...
    const isContainment =
      edge.kind === "contains" || edge.kind === "group_contains" || edge.kind.startsWith("structural_");
  ...
  if (code) {
    return {
      ...
      sentence:
        "코드 목록에서 선택한 실제 항목입니다. 저장소 안의 파일/라인을 VS Code나 Cursor에서 바로 열 수 있습니다.",
      tone: "confirmed",
      metrics: [
  ...
  if (map) {
    ...
    if (map.nodes.length === 0 && (codeItemCount > 0 || dbTableCount > 0)) {
      const firstStep =
        codeItemCount > 0 && dbTableCount > 0
          ? "코드 또는 테이블 카드 선택"
          : codeItemCount > 0
            ? "코드 카드 선택"
            : "테이블 카드 선택";
...
function columnStructureAnswer(table: DbInventoryTable, column: DbInventoryColumn): InspectorAnswer {
  return {
    ...
    metrics: [
      ...
      {
        label: "NULL",
        value: column.nullable === null || column.nullable === undefined ? "-" : column.nullable ? "허용" : "불가",
      },
    ],
    ...
    metrics: [
      ...
      ...(counts
        ? [
            {
              label: "관계",
              value: String(counts.confirmed + counts.typed + counts.inferred + counts.candidate),
              tone: counts.confirmed > 0 ? ("green" as const) : ("gray" as const),
            },
          ]
        : []),
    ],
...
function domainComposition(node: VisualNode): string {
  return node.kind === "group-domain" ? (node.subtitle?.split("|")[0] ?? "구조 영역 구성") : (node.subtitle ?? "");
}
...
function nodeAnswerSteps(node: VisualNode, map: VisualMap | null): string[] {
  ...
  if (node.kind === "column") {
    return nodeHasCodeRelation(node, map) ? ["직접 제약 먼저 확인"] : ["직접 제약 먼저 확인"];
  }
  if (node.kind === "table") {
...
function codeAnswerSteps(code: CodeInventoryItem): string[] {
  ...
  const relationStep =
    kind === "api" || kind === "route" ? "API가 닿는 코드 또는 캔버스에서 확인" : "캔버스에서 주변 관계 확인";
...
export function copyValuesForNode(node: VisualNode): Array<[string, string]> {
  ...
  if (node.kind === "file") {
    return [
      ["경로", node.subtitle ?? node.title],
      ["ID", node.id],
    ];
  }
  if (node.source === "db") {
    return [
      [nodeKindLabel(node.kind, node.source), node.title],
      ["ID", node.id],
    ];
  }
  return [
...
function nodeTrustSummary({
  ...
  if (candidateConfidence) {
    return {
      label: `후보 ${confidenceLabel(candidateConfidence) ?? "낮음"}`,
      tone: confidenceBadgeTone(candidateConfidence),
    };
  }
  if (confirmedCount > 0) {
...
export function nodeHasCodeRelation(node: VisualNode, map: VisualMap | null): boolean {
  return Boolean(
    map?.edges.some(
      (edge) => edgeTouchesNode(edge, node) && (edge.from.startsWith("code:") || edge.to.startsWith("code:")),
    ),
  );
}
...
export function columnImpactSummary(
  node: VisualNode,
  map: VisualMap | null,
): {
  directCount: number;
