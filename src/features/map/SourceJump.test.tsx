...
describe("SourceJump", () => {
  ...
  it("does not invent line one when source evidence has no position", async () => {
    render(<SourceJump workspaceId="workspace-1" code={codeItem} />);
    ...
    expect(window.localStorage.getItem("backend-visual-map:investigation:v1:workspace-1")).toContain('"line":null');

    fireEvent.click(screen.getByRole("button", { name: "VS Code" }));
    await waitFor(() =>
      expect(invoke).toHaveBeenCalledWith("open_source_location", {
        request: {
