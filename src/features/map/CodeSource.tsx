...
import { analysisReasonLabel, analysisScopeLabel, analysisStatusLabel } from "../../components/common/analysisLabels";
import { PanelHeader } from "../../components/common/PanelHeader";

export function CodeSource({ workspaceControls }: { workspaceControls: WorkspaceControls }) {
  ...
  const qualityWarning = Boolean(
    analysisQuality &&
    (analysisQuality.blindSpots.length > 0 ||
      analysisQuality.partialLanguages > 0 ||
      analysisQuality.failedLanguages > 0 ||
      codeInventory?.partial),
  );
      ...
      {analysisQuality && (
        <details className={`code-quality-details ${qualityWarning ? "warn" : "ready"}`} data-code-quality>
          <summary>
            <b>분석 품질</b>
            <span>
              언어 {analysisQuality.indexedLanguages}/{analysisQuality.languages.length} · 프레임워크{" "}
              {analysisQuality.detectedFrameworks}/{analysisQuality.frameworks.length}
            </span>
          </summary>
          <div className="code-quality-list">
            {analysisQuality.languages.map((language, index) => {
              const tone =
                language.status === "indexed" ? "ready" : language.status === "excluded" ? "excluded" : "warn";
              ...
              return (
                <span key={`language:${language.id}:${index}`} className={tone} title={detail}>
                  {language.name} · {detail}
                </span>
              );
            })}
            {analysisQuality.frameworks.map((framework, index) => (
              <span
                key={`framework:${framework.id}:${index}`}
                className={framework.status === "detected" ? "ready" : "warn"}
              >
