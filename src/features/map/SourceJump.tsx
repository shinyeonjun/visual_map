...
export function SourceJump({
  ...
  function toggleInvestigationItem(key: string) {
    updateInvestigation(
      investigationItems.map((item) => (investigationKey(item) === key ? { ...item, checked: !item.checked } : item)),
    );
  }
  ...
  return (
    <>
      <div className="copy-row" aria-label="소스 위치 열기">
        ...
        <button
          type="button"
          data-source-action="vscode"
          disabled={busyAction !== null}
          onClick={() => void openEditor("vscode")}
        >
        ...
        <button
          type="button"
          data-source-action="cursor"
          disabled={busyAction !== null}
          onClick={() => void openEditor("cursor")}
        >
          <ExternalLink size={12} />
        ...
        <button type="button" data-investigation-action="add" disabled={alreadyAdded} onClick={addInvestigationItem}>
          <Plus size={12} />
          <span>{alreadyAdded ? "조사함에 있음" : "조사함 추가"}</span>
        </button>
        {status && (
          <small role="status" data-source-status={statusTone}>
            {status}
          </small>
        )}
      </div>
      {showInvestigationTray && investigationItems.length > 0 && (
        <details className="investigation-tray-shell">
          <summary>
            <span>
              <CheckSquare size={13} />
              조사함 <b>{investigationItems.length}</b>
            </span>
          </summary>
          <section className="investigation-tray" aria-label="로컬 조사함">
            ...
            <small
              className="investigation-privacy"
              data-investigation-storage="source-location-evidence-check-state-only"
            >
