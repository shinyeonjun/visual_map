...
import { CodeSource as CodeSourceSection } from "./CodeSource";
...
describe("stable mode transitions", () => {
  it("keeps the evidence sections mounted while their values reload", () => {
    render(
      <InspectorPanel
        ...
        visualMapControls={{
          ...loadingControls("column-impact", dbMap("column-impact", "db:column:public.users:id")),
          focusId: "db:column:public.users:id",
        }}
      />,
    );

    expect(screen.getByText("선택한 대상")).toBeInTheDocument();
    expect(screen.getByText("컬럼 변경 범위")).toBeInTheDocument();
    expect(screen.getByText("다음 확인")).toBeInTheDocument();
    expect(screen.getByText("분석 업데이트 중")).toBeInTheDocument();
  });
  ...
  it("uses the requested analysis target as the default inspector subject", () => {
    ...
    expect(
      Array.from(container.querySelectorAll(".inspector-section > header > strong")).map((node) => node.textContent),
  ...
  it("lists each project once in the project switcher", () => {
    ...
    expect(screen.getAllByRole("option").map((option) => option.textContent)).toEqual(["Shop API", "Billing API"]);
  ...
  it.each([
    ...
    const headings = Array.from(container.querySelectorAll(".inspector-section > header > strong")).map(
  ...
  it("presents code engine evidence in reader-facing language", () => {
    ...
    expect(
      screen.getByText("호출 관계: bootstrap_admin 코드 항목이 _to_session_response 코드 항목을 호출합니다"),
    ).toBeInTheDocument();
...
function apiMap(): VisualMap {
  return {
    ...
    nodes: [
      {
        id: "code:route-session",
        kind: "api",
        title: "/api/v1/sessions",
        subtitle: "src/routes.ts",
        layer: "api",
        source: "code",
      },
      {
        id: "code:handler-session",
        kind: "handler",
        title: "loadSession",
        subtitle: "src/handlers.ts",
        layer: "code",
        source: "code",
      },
    ],
...
function workspaceWithApi(): WorkspaceControls {
  ...
  return {
    ...
    codeInventory: {
      ...
      handlers: [
        { id: "handler-session", kind: "handler", name: "loadSession", filePath: "src/handlers.ts", line: 24 },
      ],
    },
  } as WorkspaceControls;
}
