...
import { dbColumnNodeId, dbTableIdentityLabel, dbTableNodeId } from "../../visual/nodeIds";
...
export function buildTargetCatalog(
  ...
  const tables = [...(dbInventory?.tables ?? [])].sort((left, right) =>
    dbInventoryTableKey(left).localeCompare(dbInventoryTableKey(right)),
  );

  const uiRoutes = (codeInventory?.routes ?? []).filter(isUiRoute);
  const codeItems = [...codeInventoryCodeItems(codeInventory), ...uiRoutes, ...(codeInventory?.files ?? [])];

  return {
    api: codeInventoryBackendRoutes(codeInventory)
      .filter(isProjectCodeItem)
      .sort(compareApiTargets)
      .map((route) => ({
        id: `api:${route.id}`,
        kind: "api",
        badge: codeRouteMethod(route) ?? "API",
        title: normalizeRouteName(route.name, codeRouteMethod(route)),
...
function sortCodeTree(node: CodeTreeNode): void {
  node.children.sort(
    (left, right) =>
      Number(left.isFile) - Number(right.isFile) || left.label.localeCompare(right.label, "ko-KR", { numeric: true }),
  );
  node.items.sort(
    (left, right) =>
      left.title.localeCompare(right.title, "ko-KR", { numeric: true }) ||
...
function compareCodeTargets(
  left: CodeInventory["functions"][number],
  right: CodeInventory["functions"][number],
): number {
...
function sourceLocation(path: string | null | undefined, line: number | null | undefined): string {
  ...
  const compactPath = parts.length > 3 ? `${parts[0]}/…/${parts.slice(-2).join("/")}` : parts.join("/");
