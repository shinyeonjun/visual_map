...
export function WorkspaceSource({ workspaceControls }: { workspaceControls: WorkspaceControls }) {
    ...
    <>
      ...
      <div className="path-row">
        <input
          ...
          placeholder={workspaceControls.repoSourceMode === "github" ? "GitHub URL 입력" : "폴더 경로 입력"}
        />
      </div>
      ...
      <span className={`workspace-message ${workspaceControls.repoPathError ? "error" : ""}`}>{repoMessage}</span>
      {showNameInput && (
        <details className="source-advanced">
          <summary>프로젝트 이름: {workspaceControls.workspaceName.trim() || "자동"}</summary>
          <label className="field-label" htmlFor="workspace-name-input">
            프로젝트 이름
          </label>
          <input
  ...
  const showWorkspaceList =
    hasSavedWorkspaces && (!workspaceControls.currentWorkspace || workspaceControls.workspaces.length > 1);
    ...
    <section className="side-card workspace-source">
      ...
      {workspaceControls.recoveryWarnings.length > 0 && (
        <div className="workspace-message error" role="alert">
          {workspaceControls.recoveryWarnings.map((warning, index) => (
            <div key={warning.workspaceId + ":" + warning.kind + ":" + warning.action + ":" + index}>
