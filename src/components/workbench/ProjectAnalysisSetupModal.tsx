...
export function ProjectAnalysisSetupModal({
  ...
  return (
    <div className="analysis-setup-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section ref={dialogRef} className="analysis-setup-modal" role="dialog" aria-modal="true" aria-labelledby="analysis-setup-title" aria-describedby="analysis-setup-description">
        <header className="analysis-setup-header">
          <div>
            ...
            <p id="analysis-setup-description">{connectionOnly ? "연결 정보를 저장하고 필요할 때 DB 구조를 다시 읽을 수 있습니다." : analysisDescription}</p>
          </div>
        ...
        <div className="analysis-setup-body">
          ...
          {busy && !connectionOnly && progress && (
            <div className="analysis-progress" role="status" aria-live="polite">
              ...
              <small>{progress.determinate === false ? "분석 엔진이 작업 중입니다. 완료되면 다음 단계로 넘어갑니다." : "완료된 분석 단계 기준 진행률입니다."}</small>
            </div>
          )}
        ...
        <footer className="analysis-setup-footer">
          <span className="analysis-setup-status">{busy ? <><LoaderCircle size={14} className="spin" /> {connectionOnly ? "연결 저장 중" : `${progress?.determinate === false ? "진행 중" : `${progress?.percent ?? 0}%`} · ${progress?.label ?? "분석 준비 중"}`}</> : connectionOnly ? "연결 문자열은 저장하지 않습니다." : "선택한 범위만 분석합니다."}</span>
          <div>
