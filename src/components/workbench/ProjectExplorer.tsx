...
export function ProjectExplorer({
  ...
  const activeFocusIds = [visualMapControls.focusId ?? visualMapControls.currentMap?.focus]
  ...
  return (
    <aside className="project-explorer" aria-label="프로젝트 탐색">
      ...
      <div className="project-explorer-search">
        ...
        />
        {visualMapControls.searchPopoverOpen ? <SearchResultsPopover visualMapControls={visualMapControls} searchScope={searchScope} /> : null}
      </div>
      ...
      <footer className="explorer-footer" aria-label="핵심 보기">
        ...
        <button className={surface === "advanced" ? "active" : ""} data-surface-action="understand" type="button" onClick={() => onShowAdvanced?.("atlas")}>
          <Network size={14} />
          이해하기
        </button>
        <button className={surface === "answers" ? "active" : ""} data-surface-action="impact" type="button" onClick={onShowAnswers}>
          <GitBranch size={14} />
          영향 보기
        </button>
      </footer>
  ...
  function moveTab(event: React.KeyboardEvent<HTMLButtonElement>, current: ExplorerTab) {
    ...
    setTab(next.id);
    window.requestAnimationFrame(() => document.querySelector<HTMLButtonElement>(`[data-explorer-tab="${next.id}"]`)?.focus());
  }
}
...
function ProjectTree({
  ...
  return (
    <div className="explorer-tree-root">
      <TreeFolder label={workspaceName} open />
...
function ApiTreeItems({ items, activeFocusIds, onSelect }: { items: TargetItem[]; activeFocusIds: string[]; onSelect: (item: TargetItem) => void }) {
  ...
  return (
    <div className="explorer-api-tree">
      ...
      {items.length > API_TREE_DISPLAY_LIMIT ? (
        <p className="explorer-more">상위 {API_TREE_DISPLAY_LIMIT}개만 표시합니다. 검색으로 나머지 API를 찾을 수 있습니다.</p>
      ) : null}
    </div>
  );
}

function ApiTreeNodes({ node, activeFocusIds, onSelect }: { node: ApiTreeNode; activeFocusIds: string[]; onSelect: (item: TargetItem) => void }) {
  return (
    <div className="explorer-api-tree-branch">
      {node.children.map((child) => <ApiTreeNodeView key={child.key} node={child} activeFocusIds={activeFocusIds} onSelect={onSelect} />)}
    </div>
  );
}

function ApiTreeNodeView({ node, activeFocusIds, onSelect }: { node: ApiTreeNode; activeFocusIds: string[]; onSelect: (item: TargetItem) => void }) {
  ...
  return (
    <div className="explorer-api-node">
      <button className="explorer-api-folder" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} title={`${node.label} · ${itemCount}개 API`}>
      ...
      {open ? (
        <div className="explorer-api-children">
          {node.items.map((item) => <ApiEndpointItem key={item.id} item={item} active={activeFocusIds.includes(item.focusId)} onSelect={onSelect} />)}
...
function ApiEndpointItem({ item, active, onSelect }: { item: TargetItem; active: boolean; onSelect: (item: TargetItem) => void }) {
  const segments = apiPathSegments(item.title);
  const leaf = segments[segments.length - 1] ?? "/";
  return (
    <button className={`explorer-tree-item explorer-api-endpoint ${active ? "active" : ""}`} aria-label={item.title} aria-current={active ? "true" : undefined} type="button" onClick={() => onSelect(item)} title={`${item.title} · ${item.meta}`}>
      <span className="explorer-dot api" aria-hidden="true" />
      <span className="explorer-item-copy">
        <strong>{item.badge} /{leaf}</strong>
    ...
    <div className="explorer-tree-items">
      {items.slice(0, 80).map((item, index) => (
        <button
          ...
          onKeyDown={(event) => {
            ...
            if (next) {
              ...
              Array.from(document.querySelectorAll<HTMLButtonElement>("[data-target-id]")).find(
                (button) => button.dataset.targetId === next.focusId,
              )?.focus();
            }
          }}
        ...
        >
          ...
          <span className="explorer-item-copy">
            <strong>{item.badge} {item.title}</strong>
            <small>{item.meta}</small>
          </span>
        </button>
      ))}
...
function TreeFolder({ label, open }: { label: string; open?: boolean }) {
  return (
    <div className="explorer-tree-folder">
      <ChevronDown size={14} className={open ? "" : "closed"} />
      <Folder size={15} />
      <strong>{label}</strong>
    </div>
  );
...
function TreeSection({ icon, label, count, children }: { icon: ReactNode; label: string; count: number; children: ReactNode }) {
  return (
...
function filteredItems(catalog: TargetCatalog, kind: "api" | "code" | "table", query: string): TargetItem[] {
  ...
  return items.filter((item) => [item.badge, item.title, item.meta, item.group].some((value) => value?.toLocaleLowerCase("ko-KR").includes(query)));
}
